...
export default function DistanceTimeCalculator() {
  ...
  const pacePerMile = totals.distanceKm > 0 ? totals.seconds / (totals.distanceKm / KM_PER_MILE) : 0;
  ...
  const eta = React.useMemo(() => {
    ...
    return {
      ...
      label: `${pad(end.getHours())}:${pad(end.getMinutes())}${dayDiff > 0 ? ` (+${dayDiff}d)` : ""}`,
    };
    ...
    <div className="min-h-screen w-full bg-slate-50 text-slate-900">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <header className="flex items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Distance ➜ Time Calculator</h1>
            <p className="text-sm text-slate-600 mt-1">Convert distance to travel time for walking, running, cycling, or driving. Add multiple legs, tweak speeds, and get an ETA.</p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            />
            {eta && (
              <span className="text-sm text-slate-700">ETA: <span className="font-semibold">{eta.label}</span></span>
            )}
          </div>
        </div>
        ...
        <section className="mt-8 text-sm text-slate-600">
          ...
          <ul className="list-disc pl-5 space-y-1">
            <li>Use the dropdown presets to quickly set typical speeds, then fine-tune the speed field if needed.</li>
            <li>Adjust the <em>Conditions</em> factor to account for hills, traffic, weather, or rest stops.</li>
            <li>Add multiple legs for mixed travel (e.g., walk → train → walk) and get a single ETA.</li>
          </ul>
        </section>
  ...
  // --- Components ---
  function LegCard({ idx, leg, presets, onChange, onRemove, KM_PER_MILE }) {
    ...
    return (
      <div className="rounded-2xl bg-white border border-slate-200 shadow-sm p-4">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-700">Leg {idx + 1}</span>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                step="0.01"
                value={leg.distance}
                onChange={(e) => onChange(leg.id, { distance: safeNum(e.target.value) })}
                className="w-full px-3 py-2 rounded-xl border border-slate-300 bg-white"
              />
              <select
                className="px-3 py-2 rounded-xl border border-slate-300 bg-white"
                value={leg.distanceUnit}
                onChange={(e) => onChange(leg.id, { distanceUnit: e.target.value })}
              >
                <option value="km">km</option>
                <option value="mi">miles</option>
              </select>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                step="0.1"
                value={leg.speed}
                onChange={(e) => onChange(leg.id, { speed: safeNum(e.target.value) })}
                className="w-full px-3 py-2 rounded-xl border border-slate-300 bg-white"
              />
              <select
                className="px-3 py-2 rounded-xl border border-slate-300 bg-white"
            ...
            <div className="flex flex-wrap gap-2 mt-2 text-xs">
              <PresetSpeedButton label="Walk" valueKmH={5} onPick={(v) => onChange(leg.id, toSpeedPatch(v, leg.speedUnit))} />
              <PresetSpeedButton label="Jog" valueKmH={9} onPick={(v) => onChange(leg.id, toSpeedPatch(v, leg.speedUnit))} />
              <PresetSpeedButton label="Cycle" valueKmH={20} onPick={(v) => onChange(leg.id, toSpeedPatch(v, leg.speedUnit))} />
              <PresetSpeedButton label="City" valueKmH={30} onPick={(v) => onChange(leg.id, toSpeedPatch(v, leg.speedUnit))} />
              <PresetSpeedButton label="Motorway" valueKmH={100} onPick={(v) => onChange(leg.id, toSpeedPatch(v, leg.speedUnit))} />
            </div>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                step="1"
                value={leg.factor}
                onChange={(e) => onChange(leg.id, { factor: safeNum(e.target.value) })}
                className="w-full"
              />
              <span className="text-sm w-16 text-right">{leg.factor}%</span>
            </div>
            <p className="text-xs text-slate-500 mt-1">Adjust for hills, traffic, weather, or rest stops (100% = normal).</p>
          </div>
        </div>
        ...
        <div className="mt-3 grid grid-cols-1 lg:grid-cols-3 gap-3 items-end">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
            ...
            <div className="text-xs text-slate-500">{distanceDisplay} @ {formatSpeed(leg.speed, leg.speedUnit)}</div>
          </div>
        </div>
      </div>
    );
  ...
  function Summary({ totals, pacePerKm, pacePerMile, displayUnit }) {
    const totalDistance = displayUnit === "km" ? totals.distanceKm : totals.distanceKm / 1.609344;
    ...
    return (
      <section className="mt-6 rounded-2xl bg-white border border-slate-200 shadow-sm p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <Stat label={`Total distance (${unitLabel})`} value={fmt(totalDistance, 2)} sub={`${fmt(totals.distanceKm, 2)} km • ${fmt(totals.distanceKm / 1.609344, 2)} miles`} />
          <Stat label="Total time" value={formatHMS(totals.seconds)} sub={`${fmt(totals.seconds / 3600, 2)} hours`} />
          <Stat label="Pace (per km)" value={formatPace(pacePerKm)} sub={`/km`} />
          <Stat label="Pace (per mile)" value={formatPace(pacePerMile)} sub={`/mile`} />
        </div>
      </section>
    );
  }
  ...
  function Tests() {
    // Simple runtime test harness; renders results in a collapsible panel
    const tests = [];

    // Helpers for expected values
    const sec = (h) => Math.round(h * 3600);

    // 1) 10 km @ 5 km/h = 2h = 7200s
    tests.push({
      name: "10 km at 5 km/h",
      got: Math.round(legSeconds({ distance: 10, distanceUnit: "km", speed: 5, speedUnit: "kmh", factor: 100 })),
      want: sec(2),
    });

    // 2) 3 miles @ 3 mph = 1h = 3600s
    tests.push({
      name: "3 miles at 3 mph",
      got: Math.round(legSeconds({ distance: 3, distanceUnit: "mi", speed: 3, speedUnit: "mph", factor: 100 })),
      want: sec(1),
    });

    // 3) Conditions 150% should scale time by 1.5
    tests.push({
      name: "Conditions factor 150%",
      got: Math.round(legSeconds({ distance: 10, distanceUnit: "km", speed: 10, speedUnit: "kmh", factor: 150 })),
      want: sec(1.5),
    });

    // 4) Zero or invalid speed => 0 seconds
    tests.push({
      name: "Zero speed -> 0s",
      got: Math.round(legSeconds({ distance: 5, distanceUnit: "km", speed: 0, speedUnit: "kmh", factor: 100 })),
      want: 0,
    });

    // 5) Negative distance clamped to 0 via toKm => 0 seconds
    tests.push({
      name: "Negative distance -> 0s",
      got: Math.round(legSeconds({ distance: -5, distanceUnit: "km", speed: 5, speedUnit: "kmh", factor: 100 })),
      want: 0,
    });

    // 6) Pace format sanity: 300 sec/unit => 5:00
    ...
    return (
      <details className="mt-6 rounded-2xl border border-slate-200 bg-white p-4">
        <summary className="cursor-pointer select-none text-sm font-semibold">Built-in tests: {passed}/{tests.length} passing</summary>
        <ul className="mt-3 space-y-2 text-sm">
          {results.map((r, i) => (
            <li key={i} className={r.pass ? "text-emerald-700" : "text-red-700"}>
              <span className="font-medium">{r.pass ? "✓" : "✗"} {r.name}:</span>
              <span className="ml-2">got <code className="px-1 rounded bg-slate-100">{String(r.got)}</code> expected <code className="px-1 rounded bg-slate-100">{String(r.want)}</code></span>
            </li>
          ))}
        </ul>
      </details>
  ...
  function toSpeedPatch(valueKmH, currentUnit) {
    ...
    return { speed: valueKmH / KM_PER_MILE };
  }
}
